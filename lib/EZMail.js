...
var EZMail = function( config ) {
    ...
    if ( !cfg.host ) {
        throw new Error('EZMail: config.host must be provided');
    }
    cfg.port = parseInt('' + cfg.port, 10);
    if ( !cfg.port ) {
        throw new Error('EZMail: config.port must be provided');
    }
    if ( !cfg.username ) {
        throw new Error('EZMail: config.username must be provided');
    }
    if ( !cfg.password ) {
        throw new Error('EZMail: config.password must be provided');
    }
    if ( cfg.port !== 25 ) {
        cfg.tls = true;
    }
...
decaf.extend(EZMail.prototype, {
    ...
    send: function(message) {
        var me = this,
            thr;
        if (message.async) {
            thr =  new Thread(me.sendMail.bind(me,message));
            thr.start();
        } else {
            me.sendMail(message);
        }
    ...
    sendMail : function( message ) {
        if ( !message.from ) {
            throw new Error('EZMail.send: message.from not specified.');
        }
        if ( !message.to ) {
            throw new Error('EZMail.send: message.to not specified.');
        }
        if ( !message.subject ) {
            throw new Error('EZMail.send: message.subject not specified.');
        }
        if ( !message.text ) {
            throw new Error('EZMail.send: message.text not specified');
        }
        if ( message.attachments && !decaf.isArray(message.attachments) ) {
            throw new Error('EZMail.send: message.attachments must be an array');
        }
